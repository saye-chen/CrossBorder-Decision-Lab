...
跨域统一硬门禁：正式报告和 handoff 交付前运行 `python3 ../governance/decision-quality/validate_domain_quality.py <report> --handoff`；缺少通用决策链任一环节时只能输出 `blocked/proposed`，不得交给联合报告升级状态。
