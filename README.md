...
- CAPM includes 62 decision-distinct executable cases: 16 standalone, 14 cross-skill, 12 multi-turn, and 20 extreme scenarios. Its five full professional Golden reports each pass the 100-point gate. Production remains blocked until at least three authorized, de-identified replays include mature actual outcomes and evidence hashes, cover both creator and affiliate operations, include at least one failure or exit, and pass independent review.
...
| `creator-affiliate-partnership-management/` | CAPM skill for creator diligence, collaboration economics, negotiation, rights, affiliate operations, order reconciliation, incidents, progressive state, 62 decision-distinct executable scenarios, and controlled production gate (`CAPM-2026.07`) |
...
- CAPM 已具备 62 个决策独立可执行案例：16 个单独使用、14 个跨 Skill 联合、12 个连续追问和 20 个极端场景；5 类完整专业 Golden 报告均通过 100 分门禁。至少 3 份经授权、脱敏、包含成熟实际结果与证据哈希的回放覆盖 creator 与 affiliate，包含至少 1 个失败或退出案例并通过独立复核前，仍为受控试点。
...
| `creator-affiliate-partnership-management/` | CAPM 技能、达人尽调、合作经济、谈判交付、内容权利、联盟经营、订单守恒、事故恢复、连续状态、62个决策独立可执行场景与生产回放门（`CAPM-2026.07`，controlled pilot） |
