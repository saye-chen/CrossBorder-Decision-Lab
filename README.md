...
| 你想解决的问题 | 可先得到什么 |
|---|---|
| 广告有单但不赚钱 | 净收入、广告前后贡献、缺数与优先检查 |
| 商品值不值得小预算测 | 单位贡献、保本订单和流量、待验证门槛 |
| 现在补不补货、补多少 | 数量情景、库存缺口、排期与现金约束 |

从[首次使用教程](guides/getting-started.md)开始：安装与自检 → 合成样例 → 脱敏输入 → 一页测算卡 → 人工复核 → 结果回填。已有资料先看[字段与脱敏说明](guides/input-and-privacy.md)。[平台证据能力](guides/capability-and-evidence.md)和[真实回放评测](guides/pilot-evaluation.md)分别说明可用范围与待验证部分。
...
使用真实数据前，移除密码、API Key、客户身份和不必要的订单隐私；财务数值只在经授权的环境使用，并保留真实计算口径。具体映射和脱敏方式见[资料准备](guides/input-and-privacy.md)。当前项目仍处于 `controlled_pilot`，输出应作为决策辅助和复核材料，不应直接替代专业主体判断、审批或高风险经营决策。
...
经过授权、来源核验、去重、结果成熟与人工复盘的实际使用，才可能增加系统的长期价值：
...
先按[首次使用教程](guides/getting-started.md)运行一个样例，核对输入、输出及补数提示。
...
| 位置 | 内容 |
|---|---|
| [`guides/`](guides/getting-started.md) 与 [`examples/starter/`](examples/starter/) | 首次使用、字段映射、三任务样例和回放评测 |
