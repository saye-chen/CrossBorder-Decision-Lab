...
The system contains five independent but interoperable Codex skills:
...
按“要做什么决策”选择主 Skill，而不是按手头文件格式选择。五个 Skill 都能独立运行；跨 Skill 承接只在用户明确要求或提供上游报告时发生，不静默覆盖历史结论。
...
| `scripts/test_expert_release.py` | five Golden Reports, ten cross-skill scenarios, runtime routing, mutation, failure, and adversarial release tests |
| `evaluations/` | single-skill Golden Reports and complex cross-skill release scenarios |
...
它不是一组泛化研究提示词。CIDM 负责资本决策，CIM 负责外部竞争变化，VLB 负责内容机制与迁移性，CIG 负责一方客户证据与增量增长，D09 负责广告架构、测量、广告内部预算与放量停投。五项能力可以独立运行；跨 Skill 承接必须显式、可追溯、可逆。
...
| `scripts/test_expert_release.py` | 五份Golden Report、十个跨Skill场景、运行时路由、删减、失败和对抗发布测试 |
| `evaluations/` | 单Skill Golden Report与复杂跨Skill发布场景 |
