...
与 CIDM、CIM、VLB、CIG、D08、D09 的输入卡、允许/禁止用途、部分失败和冲突裁决读取 [skill-integration-protocol.md](references/skill-integration-protocol.md)。D07 只拥有物流库存履约主权；结构化接口读取 [data-contract-and-automation.md](references/data-contract-and-automation.md)。
...
本 Skill 保留ATP/CTP、库存分配、配送承诺、退货和容量主权。页面库存、配送、预售、退换或可购买显示需要诊断/修复时，向 `platform-store-listing-conversion` 提供对象、时点、合格可售量、承诺上限、证据ID和允许用途；D08只能据此限制或修复页面承诺，不得反向修改补货、路线或库存结论。
