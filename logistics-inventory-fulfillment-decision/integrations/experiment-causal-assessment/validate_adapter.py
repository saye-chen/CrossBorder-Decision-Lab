#!/usr/bin/env python3
"""Validate LIFD's consumer-owned ECAE adapter against the authoritative D07 contract."""
...
REPO = HERE.parents[2]
LIFD_SCRIPTS = REPO / "logistics-inventory-fulfillment-decision" / "scripts"
F01_SCRIPTS = REPO / "experiment-causal-assessment" / "scripts"
for path in (LIFD_SCRIPTS, F01_SCRIPTS):
...
def validate() -> list[str]:
    ...
    try:
        ...
        contract = contract_for(migration, "D07")
    except Exception as exc:
        return [f"authoritative_migration:{exc}"]

    exact = {
        "domain_id": "D07",
        "consumer": contract["skill"],
        "migration_id": migration["migration_id"],
        "consumer_contract_version": contract["contract_version"],
        "unknown_incremental_value_policy": "unknown_never_zero_fill",
        "external_write": False,
        "independent_owner_accepted": False,
        "production_dual_run_completed": False,
    }
    for field, expected in exact.items():
        if adapter.get(field) != expected:
            errors.append(f"adapter.{field}:expected:{expected!r}")
    if adapter.get("runtime", "").split("-", 1)[0] != contract["runtime_prefix"]:
        errors.append("adapter.runtime:domain_prefix_mismatch")
    contracted_uses = {item["use"]: item["minimum_grade"] for item in contract["use_requirements"]}
    if set(adapter.get("allowed_uses", [])) != set(contracted_uses):
        errors.append("adapter.allowed_uses:contract_mismatch")
    if adapter.get("minimum_grade_by_use") != contracted_uses:
        errors.append("adapter.minimum_grade_by_use:contract_mismatch")
    ...
    if adapter.get("legacy_labels_retained_for_archive_only") != ["C0", "C1", "C2", "C3"]:
        errors.append("adapter.legacy_labels:must_be_archive_only")
    for field in ("handoff_schema", "receipt_schema", "consumer_validator"):
        path = adapter.get(field)
        if not isinstance(path, str) or not (REPO / path).is_file():
            errors.append(f"adapter.{field}:missing")

    if acceptance.get("adapter_hash") != sha256_json(adapter):
...
if __name__ == "__main__":
    failures = validate()
    if failures:
        raise SystemExit("LIFD_ECAE_CONSUMER_ADAPTER=BLOCKED\n- " + "\n- ".join(failures))
    print("LIFD_ECAE_CONSUMER_ADAPTER=PASS local_fixture=verified controlled_pilot_owner_acceptance=accepted production_dual_run=false")
