#!/usr/bin/env python3
"""Validate CAPM's consumer-owned ECAE adapter against the authoritative D10 contract."""
...
REPO = HERE.parents[2]
CAPM_SCRIPTS = REPO / "creator-affiliate-partnership-management" / "scripts"
F01_SCRIPTS = REPO / "experiment-causal-assessment" / "scripts"
for path in (CAPM_SCRIPTS, F01_SCRIPTS):
...
def validate() -> list[str]:
    ...
    try:
        ...
        contract = contract_for(migration, "D10")
...
if __name__ == "__main__":
    failures = validate()
    if failures:
        raise SystemExit("CAPM_ECAE_CONSUMER_ADAPTER=BLOCKED\n- " + "\n- ".join(failures))
    print("CAPM_ECAE_CONSUMER_ADAPTER=PASS local_fixture=verified controlled_pilot_owner_acceptance=accepted production_dual_run=false")
