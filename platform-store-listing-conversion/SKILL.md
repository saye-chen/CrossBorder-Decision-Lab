...
正式决策、缺失数据、粘贴外部内容或潜在外部动作先读取 [交互治理协议](../governance/interaction/interaction-governance.md) 并执行 Prompt Intake Guard；仅从 ERDG 校验通过的 Decision Packet 编译 Operator Playbook。COSMO、Rufus、GEO 等动态只能使用通过校验且未过期的 [平台知识卡](../governance/platform-knowledge/platform-knowledge-contract.md) 形成诊断、假设或实验，不直接改稳定评分；Connector 按 [受控连接器治理](../governance/connectors/connector-governance.md) 提供证据，不得自动发布页面。

## 专业性与决策可用性硬约束

所有模式先读取 [professional-depth-governance.md](references/professional-depth-governance.md)。只允许压缩展示，不得删除对象/页面版本、G1—G6、证据与反对证据、根因与替代解释、具体方案、成功、停止、回滚和结果回填。

## 入口与交付层级

- Decision Card：单层页面诊断或异常。
- Decision Memo：Listing、店铺、迁移、实验或跨层改版。
- Conversion Diligence：多平台、多设备、多流量或高风险页面系统。

正式交付读取 [professional-report-delivery.md](references/output-protocols/professional-report-delivery.md)。

## 临时工作区与源文件

用户源文件、后台导出和页面素材均视为只读源，不覆盖、改名、移动或删除。复杂、可恢复任务使用 managed 模式：`${TMPDIR:-/tmp}/platform-store-listing-conversion/<YYYYMMDD-HHMMSS>-<task-slug>/`，写入包含 Skill、任务 ID 和创建时间的 `.task-owner.json`；短任务使用 `mktemp -d`。截图、页面快照、草稿和实验计算只放本任务目录。结束时只删除归属标记与任务 ID 完全匹配的目录并验证不存在；清理失败时报告准确残留路径与原因。
...
当付费流量、归因、预算、出价或放量问题涉及页面承接时，路由 `advertising-analysis-measurement-optimization`。本 Skill 向 AAMO 提交流量来源、查询/人群、素材承诺、落地页版本、到达事件、实验边界和证据 ID；AAMO 回传广告相关性和待验证机制。本 Skill 拥有页面承接主权，AAMO 拥有广告预算/出价/归因主权；页面评分不替代广告增量判断，AAMO 不得反向修改页面优化方案。AAMO 不可用时保留已验证页面事实，广告相关结论标记 `inconclusive`。

## 与物流库存履约 Skill 的边界

当页面库存显示、配送承诺、预售、退换或可购买状态需要诊断/修复时，`logistics-inventory-fulfillment-decision` 提供对象、时点、合格可售量、承诺上限、证据 ID 和允许用途；本 Skill 据此限制或修复页面承诺。本 Skill 拥有页面承接主权，LIFD 拥有库存履约主权；PLCO 不得反向修改补货、路线或库存结论。LIFD 不可用时页面库存承诺标记 `inconclusive`，不得用默认值补结论。
...
- Amazon 专项：除平台卡外，必须读取 [Amazon 店铺经营模型合同](references/amazon-store-operating-models.json)、[Amazon 店铺经营模型说明](references/amazon-store-operating-models.md) 和 [Amazon 运营流程与指标合同](references/amazon-operating-workflows-and-metrics.json)，再建立 `store_profile_id`，锁定 Seller ID/法定主体/Marketplace site/经营角色/销售项目/履约方式/库存与目录所有权/品牌授权/Ads Profile/证据时间，并为每日工作建立 `operating_archetype_id`、`overlay_ids`、`lifecycle_phase`、`cadence`、`workstream_id`、`metric_scope`、`metric_class` 和 `action_ceiling`，再运行 `scripts/validate_amazon_store_profile.py` 与 `scripts/validate_amazon_operating_board.py`。基础 archetype 只能是 3P 品牌 FBA、3P 品牌 MFN、授权经销商 FBA、授权经销商 MFN 或 Vendor Central 1P；Amazon Business、Brand Registry/Brand Store、多账户/多品牌/多站点只能作为 overlay。卖家账户、品牌资产、站点和 Offer 不得互相替代，FBA/MFN/Vendor 不得继承履约承诺，Ads attribution 不得跨 Profile 或站点继承；关键字段未知时报告只能为 conditional/inconclusive，不能写成 pass 或给出放量结论。不同店铺类型和生命周期必须使用不同运营流程与指标集合，不能用统一日报模板覆盖。
...
- `validate_plco_contract.py`：完整报告及专家级优化不可退化门；`validate_d08_contract.py` 仅为一个发布周期的旧别名兼容入口；
- `validate_decision_contract.py`：仓库共享决策合同入口的 PLCO 本地路由；
...
通过 `integrations/experiment-causal-assessment/adapter.json` 接收与本域用途、CE 门槛、payload、适用范围和有效期绑定的 F01 回执。待签、低等级、过期、范围不符、缺字段或哈希不一致时保持 `unknown`，不得把旧标签、归因或预测升级为因果/增量。F01 只提供合格证据；店铺、Listing、页面与转化动作主权仍归 PLCO，且 `external_write=false`。
